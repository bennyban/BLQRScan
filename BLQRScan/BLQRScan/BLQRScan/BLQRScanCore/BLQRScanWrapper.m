...
@interface BLQRScanWrapper ()

@property(nonatomic,strong)BLQRScanNative* scanNativeObj;   /**< ios7之后native封装 */

@property(nonatomic,strong)ZXingWrapper *scanZXingObj;      /**< ZXing封装 */

@property(nonatomic,strong)NSArray* arrayBarCodeType;       /**< 扫码类型 */

@property(nonatomic,assign)BOOL isUseZXingLib;              /**< 是否指定使用ZXing库 */

@end

@implementation BLQRScanWrapper
...
- (instancetype)initWithPreView:(UIView*)preView ArrayObjectType:(NSArray*)arrayBarCodeType cropRect:(CGRect)cropRect
                        success:(void(^)(NSArray<BLQRScanResult*> *array))blockScanResult
{
    if (self = [super init])
    {
        ...
        if ([BLQRScanWrapper isSysIos7Later])
        {
            _scanNativeObj = [[BLQRScanNative alloc] initWithPreView:preView ObjectType:arrayBarCodeType cropRect:cropRect success:^(NSArray<BLQRScanResult*> *array) {
        ...
        } else
        {
            _scanZXingObj = [[ZXingWrapper alloc]initWithPreView:preView block:^(ZXBarcodeFormat barcodeFormat, NSString *str, UIImage *scanImg) {
                ...
                if ( blockScanResult )
                {
                    NSString *barCodeType = [BLQRScanWrapper convertZXBarcodeFormat:barcodeFormat];
                    
                    BLQRScanResult *result = [[BLQRScanResult alloc]initWithScanString:str imgScan:scanImg barCodeType:barCodeType];
 ...
 */
- (instancetype)initZXingWithPreView:(UIView *)preView success:(void(^)(NSArray<BLQRScanResult*> *array))blockScanResult
{
    if (self = [super init])
    {
...
- (void)startScan
{
    if ( [BLQRScanWrapper isSysIos7Later] && !_isUseZXingLib ) {
 ...
 *  @param block 返回识别结果
 */
+ (void)recognizeImage:(UIImage*)image success:(void(^)(NSArray<BLQRScanResult*> *array))block;
{
    ...
    [ZXingWrapper recognizeImage:image block:^(ZXBarcodeFormat barCodeFormat,NSString* str)
     {
         ...
         if (block) {
             ...
             BLQRScanResult *result = [[BLQRScanResult alloc]initWithScanString:str imgScan:tmpImg barCodeType:barCodeType];
             block(@[result]);
         }
...
+ (UIImage*)createQRWithString:(NSString*)str size:(CGSize)size
{
    if ([BLQRScanWrapper isSysIos7Later]) {
        ...
        return [BLQRScanWrapper createNonInterpolatedUIImageFormCIImage:[BLQRScanWrapper createQRForString:str] withSize:size.width];
    }
...
{
    ZXBarcodeFormat zxformat = [BLQRScanWrapper convertCodeFomratToZXBarcodeFormat:format];
