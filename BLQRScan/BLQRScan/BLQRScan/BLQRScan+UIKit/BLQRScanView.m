...
@interface BLQRScanView ()

@property (nonatomic, strong) BLQRScanStyle* viewStyle;                 /**< 扫码区域各种参数 */

@property (nonatomic,assign)CGRect scanRetangleRect;                    /**< 扫码区域 */

@property (nonatomic,strong)BLQRScanLineAnimation *scanLineAnimation;   /**< 线条扫码动画封装 */

@property (nonatomic,strong)BLQRScanNetAnimation *scanNetAnimation;     /**< 网格扫码动画封装 */

@property (nonatomic,strong)UIImageView *scanLineStill;                 /**< 线条在中间位置，不移动 */

@property(nonatomic,strong)UIActivityIndicatorView* activityView;       /**< 启动相机时 菊花等待 */

@property(nonatomic,strong)UILabel *labelReadying;                      /**< 启动相机中的提示文字 */

@end

@implementation BLQRScanView
...
{
    NSInteger XRetangleLeft = _viewStyle.xScanRetangleOffset;
...
- (void)startScanAnimation
{
    ...
    {
        case BLQRScanAnimationStyle_LineMove:
        {
            //线动画
            if (!_scanLineAnimation)
                self.scanLineAnimation = [[BLQRScanLineAnimation alloc]init];
            [_scanLineAnimation startAnimatingWithRect:_scanRetangleRect
                                                InView:self
                                                 Image:_viewStyle.animationImage];
        }
            break;
        case BLQRScanAnimationStyle_NetGrid:
        {
            //网格动画
            if (!_scanNetAnimation)
                self.scanNetAnimation = [[BLQRScanNetAnimation alloc]init];
            [_scanNetAnimation startAnimatingWithRect:_scanRetangleRect
                                               InView:self
                                                Image:_viewStyle.animationImage];
        }
            break;
        case BLQRScanAnimationStyle_LineStill:
        {
            if (!_scanLineStill) {
...
- (void)drawScanRect
{
    ...
    {
        case BLQRScanPhotoframeAngleStyle_Outer:
        {
            diffAngle = linewidthAngle/3;//框外面4个角，与框紧密联系在一起
        }
            break;
        case BLQRScanPhotoframeAngleStyle_On:
        {
            diffAngle = 0;
        }
            break;
        case BLQRScanPhotoframeAngleStyle_Inner:
        {
...
{
    NSInteger XRetangleLeft = style.xScanRetangleOffset;
