...
@interface BLQRScanViewController ()

@end

@implementation BLQRScanViewController
...
- (void)drawScanView
{
    if (!_qRScanView)
    {
        ...
        self.qRScanView = [[BLQRScanView alloc]initWithFrame:rect style:_style];
...
- (void)startScan
{
    if (![BLQRScanWrapper isGetCameraPermission] )
    {
    ...
    if (!_scanObj)
    {
        ...
        {
            cropRect = [BLQRScanView getScanRectWithPreView:self.view style:_style];
        }
        _scanObj = [[BLQRScanWrapper alloc] initWithPreView:self.view
                                            ArrayObjectType:nil
                                                   cropRect:cropRect
                                                    success:^(NSArray<BLQRScanResult *> *array){
...
{
    if ([BLQRScanWrapper isGetPhotoPermission])
    {
        [self openLocalPhoto];
    } else
    {
...
- (void)scanResultWithArray:(NSArray<BLQRScanResult*>*)array
{
    if (array.count < 1) {
    ...
    for (BLQRScanResult *result in array)
    {
        NSLog(@"scanResult:%@",result.strScanned);
    }
    
    BLQRScanResult *scanResult = array[0];
    ...
    //震动提醒
    [BLQRScanWrapper systemVibrate];
    //声音提醒
    [BLQRScanWrapper systemSound];
...
- (void)showNextVCWithScanResult:(BLQRScanResult*)strResult
{
    NSLog(@"%@",strResult);
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    __weak __typeof(self) weakSelf = self;
    [BLQRScanWrapper recognizeImage:image success:^(NSArray<BLQRScanResult *> *array) {
